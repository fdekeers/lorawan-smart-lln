...
class Parser:
        ...
        for i in range(1, len(self.x)):
            ...
            if value < treshold and previous_value > treshold:
                ...
                if i-last_activation > 100:
                    self.activation_indexes += [(last_activation, i+1)]
                else:
                    self.print('Not an activation. Discarding')
                last_activation = None
                self.print('=====')
        self.print()
        self.activation_indexes = [(0, 199)]
        return self.activation_indexes
    ...
    def get_label(self):
        label = f'{self.legend}'
        #label += f'Electric charge = {round(self.compute_activation_integral(), 8)}mAh'
    ...
    def plot(self, last_activation_only=False):
        self.compute_activation_integral()
        self.y = [1000*x for x in self.y]
        plt.plot(
            self.y,
            label=self.get_label()
        )
